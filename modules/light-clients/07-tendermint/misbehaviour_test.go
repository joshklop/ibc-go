	...
	testCases := []struct {
		...
		},
		{
			...
			func(misbehaviour *ibctm.Misbehaviour) error {
				...
				tmCommit, err := tmtypes.MakeExtCommit(*blockID, int64(misbehaviour.Header2.GetHeight().GetRevisionHeight()), misbehaviour.Header1.Commit.Round, wrongVoteSet, altSignerArr, suite.now, false)
		...
		},
		{
			...
			func(misbehaviour *ibctm.Misbehaviour) error {
				...
				tmCommit, err := tmtypes.MakeExtCommit(*blockID, int64(misbehaviour.Header2.GetHeight().GetRevisionHeight()), misbehaviour.Header2.Commit.Round, wrongVoteSet, altSignerArr, suite.now, false)
