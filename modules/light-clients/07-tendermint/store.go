...
func setClientState(clientStore store.KVStore, cdc codec.BinaryCodec, clientState *ClientState) {
...
func setConsensusState(clientStore store.KVStore, cdc codec.BinaryCodec, consensusState *ConsensusState, height exported.Height) {
...
func GetConsensusState(store store.KVStore, cdc codec.BinaryCodec, height exported.Height) (*ConsensusState, bool) {
...
func deleteConsensusState(clientStore store.KVStore, height exported.Height) {
	key := host.ConsensusStateKey(height)
...
func IterateConsensusMetadata(store store.KVStore, cb func(key, val []byte) bool) {
...
func SetProcessedTime(clientStore store.KVStore, height exported.Height, timeNs uint64) {
...
func GetProcessedTime(clientStore store.KVStore, height exported.Height) (uint64, bool) {
	key := ProcessedTimeKey(height)
...
func deleteProcessedTime(clientStore store.KVStore, height exported.Height) {
	key := ProcessedTimeKey(height)
	clientStore.Delete(key)
}
...
// verification functions
func SetProcessedHeight(clientStore store.KVStore, consHeight, processedHeight exported.Height) {
...
func GetProcessedHeight(clientStore store.KVStore, height exported.Height) (exported.Height, bool) {
...
func deleteProcessedHeight(clientStore store.KVStore, height exported.Height) {
	key := ProcessedHeightKey(height)
	clientStore.Delete(key)
}
...
func SetIterationKey(clientStore store.KVStore, height exported.Height) {
...
func GetIterationKey(clientStore store.KVStore, height exported.Height) []byte {
	key := IterationKey(height)
	return clientStore.Get(key)
}

// deleteIterationKey deletes the iteration key for a given height
func deleteIterationKey(clientStore store.KVStore, height exported.Height) {
	key := IterationKey(height)
	clientStore.Delete(key)
}
...
func IterateConsensusStateAscending(clientStore store.KVStore, cb func(height exported.Height) (stop bool)) {
...
func GetNextConsensusState(clientStore store.KVStore, cdc codec.BinaryCodec, height exported.Height) (*ConsensusState, bool) {
...
func GetPreviousConsensusState(clientStore store.KVStore, cdc codec.BinaryCodec, height exported.Height) (*ConsensusState, bool) {
	iterateStore := prefix.NewStore(clientStore, []byte(KeyIterateConsensusStatePrefix))
...
func PruneAllExpiredConsensusStates(
	ctx sdk.Context, clientStore store.KVStore,
...
func getTmConsensusState(clientStore store.KVStore, cdc codec.BinaryCodec, key []byte) (*ConsensusState, bool) {
	bz := clientStore.Get(key)
	if len(bz) == 0 {
		return nil, false
	}
...
func setConsensusMetadata(ctx sdk.Context, clientStore store.KVStore, height exported.Height) {
...
func setConsensusMetadataWithValues(
	clientStore store.KVStore, height,
...
func deleteConsensusMetadata(clientStore store.KVStore, height exported.Height) {
